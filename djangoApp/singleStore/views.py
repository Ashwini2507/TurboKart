...
from .forms import CheckoutForm, CouponForm, RefundForm, PaymentForm, OrderForm, InvoiceForm, ShipmentForm, RefundForm, RefundRequestForm, ItemForm
from .models import Item, OrderItem, Order, Address, Payment, Coupon, Refund, UserProfile, Category, Brand, InvoiceItem, Invoice, ShipmentItem, Shipment,RefundItem, Refund , StoreFront
...
def products(request):
    ...
    return render(request, "singleStore/products.html", context)

class ViewCustomers(PermissionRequiredMixin, ListView):
        model = Profile
        permission_required = 'auth.view_user'
        template_name = 'singleStore/all_customers.html'
        paginate_by = 10
...
class CategoryView(ListView):
        model = Item
        template_name = 'singleStore/by_category.html'
        paginate_by = 2
...
class BrandView(ListView):
        model = Item
        template_name = 'singleStore/by_brand.html'
        paginate_by = 2
...
class CheckoutView(View):
    def get(self, *args, **kwargs):
        try:
            ...
            return render(self.request, "singleStore/checkout.html", context)
        except ObjectDoesNotExist:
            messages.info(self.request, "You do not have an active order")
            return redirect("singleStore:store-home")
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    ...
                    if address_qs.exists():
                        ...
                        return redirect('singleStore:checkout')
                else:
                ...
                if payment_option == 'S':
                    return redirect(reverse("singleStore:payment" ,kwargs = { "payment_option":"stripe"}))
                elif payment_option == 'P':
                    return redirect(reverse("singleStore:payment" ,kwargs = {"payment_option" :"paypal"}))
                else:
                    messages.warning(
            ...
            return redirect("singleStore:order-summary")
...
class PaymentView(View):
    def get(self, *args, **kwargs):
        ...
        if order.billing_address:
            ...
            return render(self.request, "singleStore/payment.html", context)
        else:
            messages.warning(
    ...
    def post(self, *args, **kwargs):
        ...
        for order_item in order.items.all():
            ...
            available_quantity = order_item.item.available_quantity
            min_stock_quantity = order_item.item.min_stock_quantity
            if request_quantity > available_quantity - min_stock_quantity:
                ...
                return redirect(reverse("singleStore:order-summary"))
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect(reverse("singleStore:store-home"))
...
class HomeView(View):
    def get(self, *args, **kwargs):
        ...
        print(context)
        return render(self.request, 'singleStore/home.html', context)
...
class OrderSummaryView(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        try:
            ...
            return render(self.request, 'singleStore/order_summary.html', context)
        except ObjectDoesNotExist:
            messages.warning(self.request, "You do not have any items in your cart yet")
            return render(self.request, 'singleStore/order_summary.html', context)
...
class ItemDetailView(View):
    ...
    def get(self, *args, **kwargs):
        try:
            ...
            return render(self.request, 'singleStore/product-page.html', context)
        except ObjectDoesNotExist:
        ...
        if form.is_valid():
            try:
                ...
                if order.coupon.code != 'Invalid Code':
                    ...
                    return redirect(reverse("singleStore:checkout"))
                else:
                    order.coupon = get_coupon(self.request, code)
...
class RequestRefundView(View):
    def get(self, *args, **kwargs):
        ...
        return render(self.request, "singleStore/request_refund.html", context)
...
class UserOrdersView(LoginRequiredMixin, ListView):
        model = Order
        template_name = 'singleStore/user_orders.html'
...
class  OrderDashboardView(PermissionRequiredMixin, ListView):
        ...
        permission_required = 'singleStore.view_order'
        template_name = 'singleStore/sales_orders.html'
        context_object_name = 'orders'
        paginate_by = 20
...
def updateOrder(request, pk):
	...
	return render(request, 'singleStore/order_form.html', context)
...
def viewInvoices(request, ref_code):
        ...
        return render(request, 'singleStore/sales_invoices.html', context)
...
def viewShipments(request, ref_code):
        ...
        return render(request, 'singleStore/sales_shipments.html', context)
...
def viewRefunds(request, ref_code):
        ...
        return render(request, 'singleStore/sales_refunds.html', context)

def viewInvoicesCustomers(request, ref_code):
        order = Order.objects.get(ref_code = ref_code)
        invoices = Invoice.objects.filter(order = order)
        context = {'invoices':invoices}
        return render(request, 'singleStore/customer_invoices.html', context)

def viewShipmentsCustomers(request, ref_code):
        order = Order.objects.get(ref_code = ref_code)
        shipments = Shipment.objects.filter(order = order)
        context = {'shipments':shipments}
        return render(request, 'singleStore/customer_shipments.html', context)

def viewRefundsCustomers(request, ref_code):
        order = Order.objects.get(ref_code = ref_code)
        refunds = Refund.objects.filter(order = order)
        context = {'refunds':refunds}
        return render(request, 'singleStore/customer_refunds.html', context)
        ...
        if request.method == 'POST':
                ...
                if form.is_valid():
                                ...
                                for order_item in order.items.all():
                                        if str((i[0].split("_")[1])) == str(order_item.item.id): 
                                                ...
                                                if order_item.invoiced_quantity + int(entry) > order_item.quantity:
                                                        ...
                                                        return redirect("singleStore:generate_invoice" ,pk = order.id)
                        ...
                        if invoice.items.all().exists():
                                ...
                                invoice.delete()
                        print(invoice.ref_code)
                        return redirect('singleStore/sales_orders')

                else:
                        print(forms.errors)
        context = {'order':order,'form':form}
        return render(request, 'singleStore/generate_invoice.html', context)

class  InvoiceDashboardView(LoginRequiredMixin, ListView):
        model = Invoice
        template_name = 'singleStore/sales_invoices.html'
...
def viewInvoice(request, pk):
        ...
        return render(request, 'singleStore/view_invoice.html', context)

def customerViewInvoice(request, ref_code):
        invoice = Invoice.objects.get(ref_code = ref_code)
        context = {'invoice':invoice}
        return render(request, 'singleStore/customer_view_invoice.html', context)
...
def ShipOrder(request, pk):
                                ...
                                for order_item in order.items.all():
                                        if str((i[0].split("_")[1])) == str(order_item.item.id): 
                                                ...
                                                if order_item.shipped_quantity + int(entry) > order_item.invoiced_quantity:
                                                        ...
                                                        return redirect("singleStore:generate_shipment" ,pk = order.id)
        ...
        context = {'order':order,'form':form}
        return render(request, 'singleStore/generate_shipment.html', context)

class  ShipmentDashboardView(LoginRequiredMixin, ListView):
        model = Shipment
        template_name = 'singleStore/sales_shipments.html'
...
def RequestRefund(request, ref_code):
        ...
        if request.method == 'POST':
                ...
                if form.is_valid():
                                        ...
                                        for order_item in order.items.all():
                                                if str((i[0].split("_")[1])) == str(order_item.item.id):
                                                        ...
                                                        if order_item.refund_request_quantity + int(entry) > order_item.quantity:
                                                                ...
                                                                return redirect("singleStore:request_refund" ,pk = order.id)
                        ...
                        order.save()
                        return redirect('singleStore:sales-orders')
        ...
        context = {'order':order,'form':form}
        return render(request, 'singleStore/request_refund.html', context)
...
def RefundOrder(request, pk):
                                ...
                                for order_item in order.items.all():
                                        if str((i[0].split("_")[1])) == str(order_item.item.id): 
                                                ...
                                                if order_item.refund_quantity + int(entry) > order_item.refund_request_quantity:
                                                        ...
                                                        return redirect("singleStore:generate_refund" ,pk = order.id)
        ...
        context = {'order':order,'form':form}
        return render(request, 'singleStore/generate_refund.html', context)

class  RefundDashboardView(LoginRequiredMixin, ListView):
        model = Refund
        template_name = 'singleStore/sales_refunds.html'
...
def viewRefund(request, pk):
        ...
        return render(request, 'singleStore/view_refund.html', context)

class  RefundRequestDashboardView(LoginRequiredMixin, ListView):
        model = Order
        template_name = 'singleStore/customer_refundRequests.html'
        context_object_name = 'refunds'
        paginate_by = 20
...
class ViewItems(LoginRequiredMixin, ListView):
        model = Item
        template_name = 'singleStore/view_products.html'
...
def updateItem(request, pk):
        ...
        if request.method == 'POST':
                ...
                if form.is_valid():
                        ...
                        return redirect("singleStore:update_item" ,pk = item.id)


        context = {'item':item,'form':form}
        return render(request, 'singleStore/update_item.html', context)
...
def createItem(request):
        ...
        if request.method == 'POST':
                ...
                if form.is_valid():
                        form.save()
                        messages.success(request, "The item has been successfully saved!")
                        return redirect("singleStore:view_items")
                else:
                        for error in form.errors:
                                messages.success(request, "The item has been successfully saved!")
                                return redirect("singleStore:create_items")

        context = {'form':form}
        return render(request, 'singleStore/create_item.html', context)
